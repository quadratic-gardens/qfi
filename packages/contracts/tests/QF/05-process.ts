...
describe("Process - Tally QV poll votes", () => {
  ...
  describe("Maci state calculations", () => {
    it("verify - correctly packs maximumVoteOptions, numsignups, currentMessageBatchIndex, indexOfCurrentBatch into a 250-bit value", async () => {
      ...
      const onChainPackedVals =
        await pollProcessorAndTallyer.genProcessMessagesPackedVals(
          poll.address,
          BigNumber.from(0),
          users.length
        )
      expect(expectedPackedVals.toString()).to.equal(
        onChainPackedVals.toString()
      )
    })
    it("verify - correctly packs the batch start index and number of signups into a 100-bit value", async () => {
      const maciExpectedPackedVals = MaciState.packTallyVotesSmallVals(
        0,
        tallyBatchSize,
        users.length
      )
      const onChainPackedVals =
        await pollProcessorAndTallyer.genTallyVotesPackedVals(
          users.length,
          0,
          tallyBatchSize
        )
      expect(onChainPackedVals.toString()).to.equal(
        maciExpectedPackedVals.toString()
      )
    })
  })
  ...
  describe("Process and Tally Vote messages", () => {
    it("verify - process messages and proof", async () => {
      ...
      const { newSbCommitment: maciNewSbCommitment } =
        maciPoll.processMessages(pollId)
      ...
      const onChainNewSbCommitment =
        await pollProcessorAndTallyer.sbCommitment()
    ...
    it("verify - tally proof is valid and updates tally commitment on chain", async () => {
      ...
      const { status: tallyVotesStatus } = await pollProcessorAndTallyer
        .tallyVotes(
          poll.address,
          maciGeneratedInputs.newTallyCommitment,
          dummyProof
        )
        .then((tx) => tx.wait())
      expect(tallyVotesStatus).to.equal(1)

      const onChainNewTallyCommitment =
        await pollProcessorAndTallyer.tallyCommitment()
      expect(maciGeneratedInputs.newTallyCommitment).to.equal(
        onChainNewTallyCommitment
      )
    })
    ...
    it("require fail - Fails if there are no untallied ballots left", async () => {
      ...
      await expect(
        pollProcessorAndTallyer.tallyVotes(
          poll.address,
          maciGeneratedInputs.newTallyCommitment,
          dummyProof
        )
    ...
    it("verify - generates tally file data", async () => {
      ...
      const onChainNewTallyCommitment =
        await pollProcessorAndTallyer.tallyCommitment()
      expect(maciTallyCircuitInputs.newTallyCommitment).to.equal(
        onChainNewTallyCommitment
      )
