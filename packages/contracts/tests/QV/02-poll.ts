...
describe("Poll - New QV Voting Round", () => {
  ...
  describe("User Sign Ups", () => {
    ...
    it("verify - allows two users to sign up to maci with correct parameters", async () => {
      ...
      const signups = await [user2, user3].map(async (user, index) => {
        ...
        const _initialVoiceCreditProxyData =
  ...
  describe("Create a new Poll", () => {
    ...
    it("verify - deploys a new poll correctly", async () => {
      ...
      await expect(
        qfi.deployPoll(_duration, _maxValues, _treeDepths, _coordinatorPubkey)
      )
        .to.emit(qfi, "DeployPoll")
        .withArgs(
          expectedPollId,
          expectedPollAddress,
          expectedCoordinatorPublicKey
        )

      const expectedTimeStamp = BigNumber.from(
      ...
      expect([...deployTimeandDuration]).to.deep.equal([
        expectedTimeStamp,
        expectedDuration
      ])
      ...
      expect([
        onChainBatchSizes.messageBatchSize,
        onChainBatchSizes.tallyBatchSize
      ]).to.deep.equal([_messageBatchSize, _tallyBatchSize])
    })
  })
})
