...
describe("Merge - Merge Signup and Message leaves", () => {
  ...
  describe("Merge and calculate tree roots", () => {
    ...
    it("verify - merge the message AccQueue", async () => {
      await expect(poll.mergeMessageAqSubRoots(0))
        .to.emit(poll, "MergeMessageAqSubRoots")
        .withArgs(0)
      await expect(poll.mergeMessageAq()).to.emit(poll, "MergeMessageAq")
    })
    it("verify - merge the MaciStateAq ", async () => {
      await expect(poll.mergeMaciStateAqSubRoots(0, 0))
        .to.emit(poll, "MergeMaciStateAqSubRoots")
        .withArgs(0)
      await expect(poll.mergeMaciStateAq(0)).to.emit(poll, "MergeMaciStateAq")
    })
    it("require fail - cannot merge messages twice", async () => {
      await expect(poll.mergeMessageAqSubRoots(0)).to.emit(
        poll,
        "MergeMessageAqSubRoots"
      )
      await expect(poll.mergeMessageAq()).to.emit(poll, "MergeMessageAq")
      await expect(poll.mergeMessageAqSubRoots(0)).to.be.revertedWith(
        "AccQueue: subtrees already merged"
      )
    })
    it("require fail - cannot merge state twice", async () => {
      await expect(poll.mergeMaciStateAqSubRoots(0, 0)).to.emit(
        poll,
        "MergeMaciStateAqSubRoots"
      )
      await expect(poll.mergeMaciStateAq(0)).to.emit(poll, "MergeMaciStateAq")
      await expect(poll.mergeMaciStateAqSubRoots(0, 0)).to.be.revertedWith(
        "PollE07"
      )
    ...
    it("verify - correctly merges messageAq", async () => {
      ...
      const expectedMessageRoot =
        maciState.polls[0].messageAq.mainRoots[
      ...
      await expect(poll.mergeMessageAqSubRoots(0))
        .to.emit(poll, "MergeMessageAqSubRoots")
        .withArgs(0)
      await expect(poll.mergeMessageAq())
        .to.emit(poll, "MergeMessageAq")
        .withArgs(expectedMessageRoot)
    })
    ...
    it("verify - correctly merges stateAq", async () => {
      ...
      const expectedStateRoot =
        maciState.stateAq.mainRoots[stateTreeDepth].toString()
