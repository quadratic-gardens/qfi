...
library PoseidonT6 {
  function hash5(uint256[5] memory input) public pure returns (uint256) {}
}
 ...
 */
abstract contract AccQueue is OwnableUpgradeable {
  ...
  uint256 public immutable subDepth;
  ...
  uint256 public currentSubtreeIndex;
  ...
  // Subtree roots
  mapping(uint256 => uint256) public subRoots;

  // Merged roots
  uint256[MAX_DEPTH + 1] public mainRoots;
  ...
  constructor(uint256 _subDepth, uint256 _hashLength) {
    require(_subDepth > 0, "AccQueue: _subDepth must be more than 0");
    require(_subDepth <= MAX_DEPTH, "AccQueue: only supports up to MAX_DEPTH levels");
    require(_hashLength == 2 || _hashLength == 5, "AccQueue: only supports _hashLength of 2 or 5");
   ...
   * contract uses will be either PoseidonT3.hashLeftRight or PoseidonT6.hash5, which require
   ...
   * This is a virtual function as the hash function which the overriding
   * contract uses will be either PoseidonT3.hashLeftRight or PoseidonT6.hash5, which will produce
   * different zero values (e.g. PoseidonT3.hashLeftRight(0, 0) vs
   * PoseidonT6.hash5([0, 0, 0, 0, 0]). Moreover, the zero value may be a
  ...
  function queueSubRoot(uint256 _leaf, uint256 _level, uint256 _maxDepth) internal {
    ...
    if (n != hashLength - 1) {
      ...
      if (isBinary) {
        ...
        inputs[1] = _leaf;
        hashed = PoseidonT3.hash2(inputs);
      } else {
        ...
        inputs[n] = _leaf;
        hashed = PoseidonT6.hash5(inputs);
      }
  ...
  function merge(uint256 _depth) public onlyOwner returns (uint256) {
    ...
    require(_depth >= srtDepth, "AccQueue: _depth must be gte the SRT depth");
  ...
  function getSubRoot(uint256 _index) public view returns (uint256) {
    require(currentSubtreeIndex > _index, "AccQueue: _index must refer to a complete subtree");
  ...
  function getMainRoot(uint256 _depth) public view returns (uint256) {
    require(hashLength ** _depth >= numLeaves, "AccQueue: getMainRoot: _depth must be high enough");
...
abstract contract AccQueueBinary is AccQueue {
  ...
  function hashLevel(uint256 _level, uint256 _leaf) internal override returns (uint256) {
    uint256 hashed = PoseidonT3.hashLeftRight(leafQueue.levels[_level][0], _leaf);
  ...
  function hashLevelLeaf(uint256 _level, uint256 _leaf) public view override returns (uint256) {
    uint256 hashed = PoseidonT3.hashLeftRight(leafQueue.levels[_level][0], _leaf);
    return hashed;
  }
...
abstract contract AccQueueQuinary is AccQueue {
  ...
  function hashLevel(uint256 _level, uint256 _leaf) internal override returns (uint256) {
    ...
    inputs[4] = _leaf;
    uint256 hashed = PoseidonT6.hash5(inputs);
