...
contract OptimisticRecipientRegistry is Ownable, BaseRecipientRegistry {
  ...
  // Events
  event RequestSubmitted(bytes32 indexed _recipientId, RequestType indexed _type, address _recipient, string _metadata, uint256 _timestamp);
  ...
  function addRecipient(address _recipient, string calldata _metadata) external payable {
    ...
    requests[recipientId] = Request(RequestType.Registration, payable(msg.sender), block.timestamp, msg.value, _recipient, _metadata);
  ...
  function removeRecipient(bytes32 _recipientId) external payable {
    ...
    require(msg.value == baseDeposit, "RecipientRegistry: Incorrect deposit amount");
    requests[_recipientId] = Request(RequestType.Removal, payable(msg.sender), block.timestamp, msg.value, address(0), "");
  ...
  function executeRequest(bytes32 _recipientId) external returns (bool) {
    ...
    require(request.submissionTime != 0, "RecipientRegistry: Request does not exist");
    if (msg.sender != owner()) {
      require(block.timestamp - request.submissionTime >= challengePeriodDuration, "RecipientRegistry: Challenge period is not over");
    }
