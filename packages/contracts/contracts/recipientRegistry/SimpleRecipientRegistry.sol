...
contract SimpleRecipientRegistry is Ownable, BaseRecipientRegistry {
  ...
  event RecipientRemoved(bytes32 indexed _recipientId, uint256 _timestamp);
   ...
   */
  function addRecipient(address _recipient, string calldata _metadata) external onlyOwner {
   ...
   */
  function removeRecipient(bytes32 _recipientId) external onlyOwner {
