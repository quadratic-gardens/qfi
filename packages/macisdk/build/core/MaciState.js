...
import { PubKey, PCommand, TCommand, Message, Keypair, StateLeaf, Ballot } from "../domainobjs/domainobjs";
import { AccQueue, IncrementalQuinTree, genRandomSalt, SNARK_FIELD_SIZE, NOTHING_UP_MY_SLEEVE, hashLeftRight, hash3, hash5, sha256Hash, stringifyBigInts } from "../crypto";
...
class Poll {
    ...
    VOTE_OPTION_TREE_ARITY = 5;
    stateCopied = false;
    stateLeaves = [blankStateLeaf];
    stateTree = new IncrementalQuinTree(STATE_TREE_DEPTH, blankStateLeafHash, this.STATE_TREE_ARITY, hash5);
    ...
    subsidy = []; // size: M, M is number of vote options
    subsidySalts = {};
    rbi = 0; // row batch index
    cbi = 0; // column batch index
    MM = 50; // adjustable parameter
    WW = 4; // number of digits for float representation
    constructor(_duration, _pollEndTimestamp, _coordinatorKeypair, _treeDepths, _batchSizes, _maxValues, _maciStateRef) {
        ...
        this.messageTree = new IncrementalQuinTree(this.treeDepths.messageTreeDepth, NOTHING_UP_MY_SLEEVE, this.MESSAGE_TREE_ARITY, hash5);
    ...
    copyStateFromMaci = () => {
        ...
        this.ballotTree = new IncrementalQuinTree(STATE_TREE_DEPTH, emptyBallot.hash(), this.STATE_TREE_ARITY, hash5);
    ...
    processMessages = (_pollId) => {
        ...
        const circuitInputs = stringifyBigInts(this.genProcessMessagesCircuitInputsPartial(this.currentMessageBatchIndex));
        ...
        circuitInputs.inputHash = sha256Hash([circuitInputs.packedVals, coordPubKeyHash, circuitInputs.msgRoot, circuitInputs.currentSbCommitment, circuitInputs.newSbCommitment, this.pollEndTimestamp]);
     ...
     */
    genProcessMessagesCircuitInputsPartial = (_index) => {
        ...
        const currentSbCommitment = hash3([currentStateRoot, currentBallotRoot, this.sbSalts[this.currentMessageBatchIndex]]);
        // Generate a SHA256 hash of inputs which the contract provides
        const packedVals = BigInt(this.maxValues.maxVoteOptions) + (BigInt(this.numSignUps) << BigInt(50)) + (BigInt(_index) << BigInt(100)) + (BigInt(batchEndIndex) << BigInt(150));
    ...
    processMessage = (_index) => {
        ...
        try {
            ...
            const voiceCreditsLeft = BigInt(`${stateLeaf.voiceCreditBalance}`) + BigInt(`${prevSpentCred}`) * BigInt(`${prevSpentCred}`) - BigInt(`${command.newVoteWeight}`) * BigInt(`${command.newVoteWeight}`);
    ...
    subsidyPerBatch = () => {
        ...
        const packedVals = MaciState.packSubsidySmallVals(this.rbi, this.cbi, this.numSignUps);
    ...
    coefficientCalculation = (rowBallot, colBallot) => {
        let sum = BigInt(0);
    ...
    subsidyCalculation = (rowStartIndex, colStartIndex) => {
        const batchSize = this.batchSizes.subsidyBatchSize;
        let ballots1 = [];
        let ballots2 = [];
        const emptyBallot = new Ballot(this.maxValues.maxVoteOptions, this.treeDepths.voteOptionTreeDepth);
    ...
    tallyVotes = () => {
        ...
        const currentPerVOSpentVoiceCreditsRootSalt = batchStartIndex === 0 ? BigInt(0) : this.preVOSpentVoiceCreditsRootSalts[batchStartIndex - batchSize];
        const currentSpentVoiceCreditSubtotalSalt = batchStartIndex === 0 ? BigInt(0) : this.spentVoiceCreditSubtotalSalts[batchStartIndex - batchSize];
        const currentResultsCommitment = this.genResultsCommitment(currentResultsRootSalt);
        const currentPerVOSpentVoiceCreditsCommitment = this.genPerVOSpentVoiceCreditsCommitment(currentPerVOSpentVoiceCreditsRootSalt, batchStartIndex);
        const currentSpentVoiceCreditsCommitment = this.genSpentVoiceCreditSubtotalCommitment(currentSpentVoiceCreditSubtotalSalt, batchStartIndex);
        const currentTallyCommitment = batchStartIndex === 0 ? BigInt(0) : hash3([currentResultsCommitment, currentSpentVoiceCreditsCommitment, currentPerVOSpentVoiceCreditsCommitment]);
        const ballots = [];
        ...
        const newSpentVoiceCreditsCommitment = this.genSpentVoiceCreditSubtotalCommitment(newSpentVoiceCreditSubtotalSalt, batchStartIndex + batchSize);
        const newPerVOSpentVoiceCreditsCommitment = this.genPerVOSpentVoiceCreditsCommitment(newPerVOSpentVoiceCreditsRootSalt, batchStartIndex + batchSize);
        const newTallyCommitment = hash3([newResultsCommitment, newSpentVoiceCreditsCommitment, newPerVOSpentVoiceCreditsCommitment]);
        //debugger
        const stateRoot = this.stateTree.root;
        const ballotRoot = this.ballotTree.root;
        const sbSalt = this.sbSalts[this.currentMessageBatchIndex];
        const sbCommitment = hash3([stateRoot, ballotRoot, sbSalt]);
        const packedVals = MaciState.packTallyVotesSmallVals(batchStartIndex, batchSize, this.numSignUps);
    ...
    genResultsCommitment = (_salt) => {
        const resultsTree = new IncrementalQuinTree(this.treeDepths.voteOptionTreeDepth, BigInt(0), this.VOTE_OPTION_TREE_ARITY, hash5);
        for (const r of this.results) {
            resultsTree.insert(r);
        }
        return hashLeftRight(resultsTree.root, _salt);
    };
    genSpentVoiceCreditSubtotalCommitment = (_salt, _numBallotsToCount) => {
    ...
    //}
    genPerVOSpentVoiceCreditsCommitment = (_salt, _numBallotsToCount) => {
        const resultsTree = new IncrementalQuinTree(this.treeDepths.voteOptionTreeDepth, BigInt(0), this.VOTE_OPTION_TREE_ARITY, hash5);
        const leaves = [];
    ...
    copy = () => {
        const copied = new Poll(Number(this.duration.toString()), BigInt(this.pollEndTimestamp.toString()), this.coordinatorKeypair.copy(), {
        ...
        copied.subsidy = this.subsidy.map((x) => BigInt(x.toString()));
    ...
    equals = (p) => {
        const result = this.duration === p.duration &&
...
class MaciState {
    ...
    stateLeaves = [];
    stateTree = new IncrementalQuinTree(STATE_TREE_DEPTH, blankStateLeafHash, this.STATE_TREE_ARITY, hash5);
    stateAq = new AccQueue(this.STATE_TREE_SUBDEPTH, this.STATE_TREE_ARITY, blankStateLeafHash);
    pollBeingProcessed = true;
    ...
    signUp(_pubKey, _initialVoiceCreditBalance, _timestamp) {
    ...
    deployPoll(_duration, _pollEndTimestamp, _maxValues, _treeDepths, _messageBatchSize, _coordinatorKeypair) {
        const poll = new Poll(_duration, _pollEndTimestamp, _coordinatorKeypair, _treeDepths, {
    ...
    equals = (m) => {
        const result = this.STATE_TREE_ARITY === m.STATE_TREE_ARITY && this.MESSAGE_TREE_ARITY === m.MESSAGE_TREE_ARITY && this.VOTE_OPTION_TREE_ARITY === m.VOTE_OPTION_TREE_ARITY && this.stateTreeDepth === m.stateTreeDepth && this.polls.length === m.polls.length && this.stateLeaves.length === m.stateLeaves.length;
        if (!result) {
            return false;
        }
    ...
    static packProcessMessageSmallVals = (maxVoteOptions, numUsers, batchStartIndex, batchEndIndex) => {
        return BigInt(`${maxVoteOptions}`) + (BigInt(`${numUsers}`) << BigInt(50)) + (BigInt(batchStartIndex) << BigInt(100)) + (BigInt(batchEndIndex) << BigInt(150));
    };
    static unpackProcessMessageSmallVals = (packedVals) => {
        let asBin = packedVals.toString(2);
...
const genProcessVkSig = (_stateTreeDepth, _messageTreeDepth, _voteOptionTreeDepth, _batchSize) => {
    return (BigInt(_batchSize) << BigInt(192)) + (BigInt(_stateTreeDepth) << BigInt(128)) + (BigInt(_messageTreeDepth) << BigInt(64)) + BigInt(_voteOptionTreeDepth);
};
const genTallyVkSig = (_stateTreeDepth, _intStateTreeDepth, _voteOptionTreeDepth) => {
    return (BigInt(_stateTreeDepth) << BigInt(128)) + (BigInt(_intStateTreeDepth) << BigInt(64)) + BigInt(_voteOptionTreeDepth);
};
const genSubsidyVkSig = (_stateTreeDepth, _intStateTreeDepth, _voteOptionTreeDepth) => {
    return (BigInt(_stateTreeDepth) << BigInt(128)) + (BigInt(_intStateTreeDepth) << BigInt(64)) + BigInt(_voteOptionTreeDepth);
};
/*
 ...
 */
const genTallyResultCommitment = (results, salt, depth) => {
...
export { MaciState, Poll, genProcessVkSig, genTallyVkSig, genSubsidyVkSig, genTallyResultCommitment, STATE_TREE_DEPTH };
