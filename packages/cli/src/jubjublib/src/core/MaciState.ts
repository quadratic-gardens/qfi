...
import { PubKey, VerifyingKey, Command, PCommand, TCommand, Message, Keypair, StateLeaf, Ballot } from "../domainobjs/domainobjs.js";
import { AccQueue, IncrementalQuinTree, genRandomSalt, SNARK_FIELD_SIZE, NOTHING_UP_MY_SLEEVE, hashLeftRight, hash3, hash5, sha256Hash, stringifyBigInts, Signature } from "../crypto/index.js";
...
class Poll {
  ...
  public stateTree = new IncrementalQuinTree(STATE_TREE_DEPTH, blankStateLeafHash, this.STATE_TREE_ARITY, hash5);
  ...
  constructor(_duration: number, _pollEndTimestamp: BigInt, _coordinatorKeypair: Keypair, _treeDepths: TreeDepths, _batchSizes: BatchSizes, _maxValues: MaxValues, _maciStateRef: MaciState) {
...
class MaciState {
  ...
  public deployPoll(_duration: number, _pollEndTimestamp: BigInt, _maxValues: MaxValues, _treeDepths: TreeDepths, _messageBatchSize: number, _coordinatorKeypair: Keypair): number {
  ...
  public equals = (m: MaciState): boolean => {
    const result =
      this.STATE_TREE_ARITY === m.STATE_TREE_ARITY && this.MESSAGE_TREE_ARITY === m.MESSAGE_TREE_ARITY && this.VOTE_OPTION_TREE_ARITY === m.VOTE_OPTION_TREE_ARITY && this.stateTreeDepth === m.stateTreeDepth && this.polls.length === m.polls.length && this.stateLeaves.length === m.stateLeaves.length;
  ...
  public static packProcessMessageSmallVals = (maxVoteOptions: BigInt, numUsers: BigInt, batchStartIndex: number, batchEndIndex: number) => {
...
const genProcessVkSig = (_stateTreeDepth: number, _messageTreeDepth: number, _voteOptionTreeDepth: number, _batchSize: number): BigInt => {
...
export { type MaxValues, type TreeDepths, MaciState, Poll, genProcessVkSig, genTallyVkSig, genSubsidyVkSig, genTallyResultCommitment, STATE_TREE_DEPTH };
