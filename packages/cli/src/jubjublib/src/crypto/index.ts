...
import {utils as ffutils, Scalar} from "ffjavascript";
import createBlakeHash from "blake-hash";
import { babyJub, poseidon, eddsa } from "circomlib";
import { OptimisedMT as IncrementalQuinTree } from "../optimisedmt/index.js";

import { AccQueue } from "./AccQueue.js";
import { poseidonEncrypt, poseidonDecrypt } from "../poseidonencryption/index.js";
...
const stringifyBigInts: (obj: object) => any = ffutils.stringifyBigInts;
const unstringifyBigInts: (obj: object) => any = ffutils.unstringifyBigInts;

type SnarkBigInt = BigInt;
type IPrivKey = BigInt;
type IPubKey = BigInt[];
...
const NOTHING_UP_MY_SLEEVE = BigInt(utils.soliditySha256(["bytes"], [utils.toUtf8Bytes("Maci")])) % SNARK_FIELD_SIZE;
...
const hash13 = (elements: Plaintext): BigInt => {
  ...
  return poseidonT6([elementsPadded[0], poseidonT6(elementsPadded.slice(1, 6)), poseidonT6(elementsPadded.slice(6, 11)), elementsPadded[11], elementsPadded[12]]);
};
 ...
 */
const formatPrivKeyForBabyJub = (privKey: IPrivKey) => {
  const sBuff = eddsa.pruneBuffer(createBlakeHash("blake512").update(bigInt2Buffer(privKey)).digest().slice(0, 32));
 ...
 */
const packPubKey = (pubKey: IPubKey): Buffer => {
 ...
 */
const unpackPubKey = (packed: Buffer): IPubKey => {
...
const genKeypair = (): IKeypair => {
  const privKey = genPrivKey();
  const pubKey = genPubKey(privKey);

  const Keypair: IKeypair = { privKey, pubKey };
 ...
 */
const genEcdhSharedKey = (privKey: IPrivKey, pubKey: IPubKey): EcdhSharedKey => {
 ...
 */
const encrypt = (plaintext: Plaintext, sharedKey: EcdhSharedKey, nonce = BigInt(0)): Ciphertext => {
 ...
 */
const decrypt = (ciphertext: Ciphertext, sharedKey: EcdhSharedKey, nonce: BigInt, length: number): Plaintext => {
 ...
 */
const sign = (privKey: IPrivKey, msg: BigInt): Signature => {
 ...
 */
const verifySignature = (msg: BigInt, signature: Signature, pubKey: IPubKey): boolean => {
